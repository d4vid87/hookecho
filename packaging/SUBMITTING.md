...
- `Cargo.lock` is committed, so the sources file describes the same crates a
  fresh checkout resolves. The `flatpak_sources` test in
  `crates/hookecho/tests/` enforces that: every registry crate in the lockfile
