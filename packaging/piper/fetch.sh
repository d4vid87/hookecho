...
readarray -t model < <(jq -r '.voice.model[]' "$manifest")
readarray -t config < <(jq -r '.voice.config[]' "$manifest")
...
readarray -t archive < <(jq -r --arg p "$platform" '.piper.archives[$p][]' "$manifest")
