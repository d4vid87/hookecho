...
| C radar analysis | Partial | derived radar products and detectors; versioned portable product definitions have a live-validating settings editor, are saved/imported/exported/synced with settings, and compile to a typed CPU AST with declared radar/environment inputs and deterministic limits; gate-local products use normal pane rendering and sampling; a bounded, order-independent min/max trail engine applies threshold/window/reset controls, retains contributor age, and exports coordinate/time/value CSV; shared beam-coverage logic reports half-power envelopes, terrain blockage, lowest usable tilt, and the best lower-beam neighboring radar in the map measure UI | DSL parser/type/resource/round-trip/scalar/column/aligned-sweep tests, trail order/window/reset/grid/export tests, and beam envelope/blockage/comparison tests pass; trail rendering and GPU expression parity remain pending |
...
| L route/chase | Partial | GPS, packs, sharing, archive replay, user-configured OSRM route requests, alternate-route display, distance/ETA, active-warning intersection distance, 1 km native-value profiles for the visible registered field (including MRMS/model fields) or native single-radar sweep, and selected-storm closest approach/relative bearing/route-vs-storm ETA with a crossing marker | Route request/GeoJSON validation, bounded geometry, polygon-hole, profile-spacing, native radar sampling, and synchronized storm-crossing tests, native Clippy, settings compatibility, and WASM checks pass; pending forecast-time exposure and pack v2 |
| M output/API | Partial | snapshots, GIF/MP4 with fixed or exact source-time intervals and JSON provenance sidecars, streamer mode, timestamp-exact archive loop selection, streaming native-value field CSV with coordinates/units/timestamps/provenance, and a token-protected versioned local API for radar/product discovery, frame provenance, native point probes, cells, health, stills, and loops while preserving legacy routes | Exact archive-time selection, interval encoding, sidecar schema, scientific CSV, and local API route/catalog tests, native Clippy, and WASM checks pass; pending broadcast presets and feasibility-gated CF/Radial/GeoTIFF formats |
