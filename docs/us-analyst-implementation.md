...
| M output/API | Partial | snapshots, GIF/MP4, streamer mode, timestamp-exact archive loop selection, and a token-protected versioned local API for radar/product discovery, frame provenance, native point probes, cells, health, stills, and loops while preserving legacy routes | Exact archive-time selection and local API route/catalog tests, native Clippy, and WASM checks pass; pending broadcast presets, source/fixed interval encoding, metadata sidecars, and scientific formats |
