...
| C radar analysis | Partial | derived radar products and detectors; versioned portable product definitions compile to a typed CPU AST with declared radar/environment inputs, deterministic expression/column limits, safe missing-data behavior, arithmetic, masks, thresholds, min/max/mean, clamp, vertical/layer reducers, height crossings, and gate counts | Parser/type/resource/round-trip/scalar/column tests pass; normal-pane rendering, sampling, and GPU parity remain pending |
