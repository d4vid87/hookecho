...
| C radar analysis | Partial | derived radar products and detectors; versioned portable product definitions are saved/exported/synced with settings and compile to a typed CPU AST with declared radar/environment inputs, deterministic expression/column limits, safe missing-data behavior, arithmetic, masks, thresholds, min/max/mean, clamp, vertical/layer reducers, height crossings, and gate counts; gate-local products produce the same bounded polar sweep format used by normal radar panes | Parser/type/resource/product/settings round-trip/scalar/column/aligned-sweep tests pass; pane selection, product-native sampling, and GPU parity remain pending |
