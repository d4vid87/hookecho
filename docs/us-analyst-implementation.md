...
| C radar analysis | Partial | derived radar products and detectors; versioned portable product definitions are saved/exported/synced with settings and compile to a typed CPU AST with declared radar/environment inputs, deterministic expression/column limits, safe missing-data behavior, arithmetic, masks, thresholds, min/max/mean, clamp, vertical/layer reducers, height crossings, and gate counts; gate-local products use the normal radar GPU upload, cursor sampling, product units/provenance, menu selection, and workspace persistence | Parser/type/resource/product/settings/workspace round-trip/scalar/column/aligned-sweep tests pass; definition editor/import and GPU parity remain pending |
