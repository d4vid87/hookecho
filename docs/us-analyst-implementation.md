...
| C radar analysis | Partial | derived radar products and detectors; versioned portable product definitions compile to a typed CPU AST with declared radar/environment inputs, deterministic size/depth limits, safe missing-data behavior, arithmetic, masks, thresholds, min/max/mean, and clamp | Parser/type/resource/round-trip/evaluation tests pass; vertical reducers, normal-pane rendering, sampling, and GPU parity remain pending |
