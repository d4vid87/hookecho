...
| J 3D | Partial | GPU volume raymarch, cross sections, CAPPI slices, six selectable radar moments with per-moment palettes/units, quality budgets, threshold transfer, and three-axis clipping; camera-only changes update uniforms without rebuilding the volume | 3D state/threshold/quality tests, native Clippy, and WASM checks pass; explicit isosurface mode remains pending |
