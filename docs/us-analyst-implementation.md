...
| B operational radar | Partial | AWS live-chunk provider adapter with immediate complete-volume archive fallback, progressive radial-block merge, six moments, VCP/cut/radial/latency/retry telemetry | Reordered/duplicate/missing-block and fallback-state tests pass; realtime chunks exactly match the same archived KDMX Level II fixture |
