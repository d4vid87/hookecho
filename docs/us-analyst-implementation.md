...
|---|---|---|---|
| A1 field registry | Partial | `wxdata::field`; MRMS descriptors; stable workspace/favorite/recent IDs with legacy-slug reads; descriptor-backed search and native-value map sampling | `cargo test -p wxdata --lib`; workspace, descriptor coverage, field-ID, history and search tests |
