...
| L route/chase | Partial | GPS, packs, sharing, archive replay, user-configured OSRM route requests, alternate-route display, distance/ETA, active-warning intersection distance, 1 km native-value profiles for the visible registered field (including MRMS/model fields), and selected-storm closest approach/relative bearing/route-vs-storm ETA with a crossing marker | Route request/GeoJSON validation, bounded geometry, polygon-hole, profile-spacing, and synchronized storm-crossing tests, native Clippy, settings compatibility, and WASM checks pass; pending single-radar/lightning/forecast-time exposure and pack v2 |
| M output/API | Partial | snapshots, GIF/MP4 with fixed or exact source-time intervals, streamer mode, timestamp-exact archive loop selection, and a token-protected versioned local API for radar/product discovery, frame provenance, native point probes, cells, health, stills, and loops while preserving legacy routes | Exact archive-time selection, interval encoding, and local API route/catalog tests, native Clippy, and WASM checks pass; pending broadcast presets, metadata sidecars, and scientific formats |
