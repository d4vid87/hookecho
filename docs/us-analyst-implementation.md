...
| D satellite/GLM | Partial | GLM flashes and registry-backed flash-extent density; all 16 native ABI CMIP channels plus declarative C02/C03/C01 true-color rendering across selectable CONUS, Mesoscale 1, and Mesoscale 2 scenes; catalog channels generate search rows, source dispatch, palettes, timeline following, and offline-pack bands from metadata; fixed-grid navigation, quality masking, native sampling, unequal-channel-grid reprojection, automatic reload cache, and timeline-complete pinned web packs | Real GOES-19 C13 mesoscale fixture passes value, DQF, timestamp, forward/inverse projection, native sample, and display-frame checks; all-channel/scene registry, RGB quality/time/grid, direct-RGBA upload, ABI object-time, cache LRU/pinning, and pack-manifest tests pass; live cross-source release capture pending |
