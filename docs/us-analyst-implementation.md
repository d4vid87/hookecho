...
| L route/chase | Partial | GPS, packs, sharing, archive replay, user-configured OSRM route requests, alternate-route display, distance/ETA, active-warning intersection distance, and 1 km native-value profiles for the visible registered field (including MRMS/model fields) | Route request/GeoJSON validation, bounded geometry, polygon-hole and profile-spacing tests, native Clippy, settings compatibility, and WASM checks pass; pending single-radar/lightning/forecast-time exposure, intercept geometry, and pack v2 |
