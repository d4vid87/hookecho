...
fn nst_matches_metpy_golden() {
    ...
    let mut mine: Vec<_> = p
        .cells
        .iter()
        .map(|c| (c.id.clone(), c.x_km, c.y_km))
        .collect();
    ...
    assert!(
        graphic.contains("DBZM"),
        "graphic has DBZM line: {:?}",
        &graphic[..graphic.len().min(80)]
    );
    ...
    assert!(
        p.past_tracks.iter().all(|t| t.len() >= 2),
...
fn eet_matches_metpy_golden() {
    ...
    for s in &g.samples {
        ...
        match (nexrad_level3::eet_value(level, &p.thresholds), s.value) {
            (Some((kft, _topped)), Some(want)) => {
                assert!((kft - want).abs() < 0.5, "EET value {kft} vs {want}")
            }
            (None, None) => {}
