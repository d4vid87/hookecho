...
//! Low CC and high reflectivity alone can also occur in hail. A TDS hit additionally requires a
//! nearby low-level velocity couplet from the same radar and elevation.
//!
...
/// clustered on a ~4 km grid; clusters with `>= min_gates` gates become hits only within 8 km
/// of a valid same-tilt velocity couplet. `vel` must be dealiased.
pub fn detect(
    z: &BinnedSweep,
    cc: &BinnedSweep,
    vel: &BinnedSweep,
    ...
    if cc.az_bins == 0
        || cc.gate_count == 0
        || z.gate_count == 0
        || cc.az_bins != z.az_bins
        || (cc.elevation_deg - z.elevation_deg).abs() > 0.15
        || (cc.elevation_deg - vel.elevation_deg).abs() > 0.15
        || cc.radar_lat != z.radar_lat
        || cc.radar_lon != z.radar_lon
        || cc.radar_lat != vel.radar_lat
        || cc.radar_lon != vel.radar_lon
        || vel.moment != Moment::Velocity
    {
        return Vec::new();
    }
    // NWS TDS guidance requires a valid circulation collocated with the dual-pol signature.
    // Dealiased velocity avoids a folded gate manufacturing a false couplet; the near-range
    // floor rejects clutter whose azimuth bins are only a few hundred metres apart.
    let couplets = crate::rotation::detect(vel, 25.0, 15.0, max_range_km, 3);
    ...
    let mut hits: Vec<TdsHit> = cells
        ...
        .filter(|hit| {
            hit.gates >= min_gates
                && couplets.iter().any(|rotation| {
                    let dlat = (hit.lat - rotation.lat).to_radians();
                    let dlon = (hit.lon - rotation.lon).to_radians();
                    let a = (dlat / 2.0).sin().powi(2)
                        + hit.lat.to_radians().cos()
                            * rotation.lat.to_radians().cos()
                            * (dlon / 2.0).sin().powi(2);
                    12_742.0 * a.sqrt().asin() <= 8.0
                })
        })
        .collect();
...
mod tests {
    ...
    fn velocity_couplet(azimuth: usize) -> BinnedSweep {
        let mut velocity = sweep(Moment::Velocity, idx(Moment::Velocity, 0.0), 0, 0..0, 0..0);
        for gate in 40..60 {
            for az in azimuth..azimuth + 4 {
                velocity.data[az * velocity.gate_count + gate] = idx(Moment::Velocity, -30.0);
            }
            for az in azimuth + 4..azimuth + 8 {
                velocity.data[az * velocity.gate_count + gate] = idx(Moment::Velocity, 30.0);
            }
        }
        velocity
    ...
    fn flags_low_cc_in_high_z() {
        ...
        let hits = detect(&z, &cc, &velocity_couplet(100), 0.80, 40.0, 150.0, 4);
    ...
    fn ignores_low_cc_in_weak_echo() {
        ...
        assert!(detect(&z, &cc, &velocity_couplet(100), 0.80, 40.0, 150.0, 4).is_empty());
    }

    #[test]
    fn low_cc_and_high_reflectivity_without_collocated_rotation_is_not_tds() {
        let cc = sweep(
            Moment::CorrelationCoefficient,
        ...
        let no_rotation = sweep(Moment::Velocity, idx(Moment::Velocity, 0.0), 0, 0..0, 0..0);
        assert!(detect(&z, &cc, &no_rotation, 0.80, 40.0, 150.0, 4).is_empty());
        assert!(detect(&z, &cc, &velocity_couplet(250), 0.80, 40.0, 150.0, 4).is_empty());
    }
}
